...
If you are unsure whether you will install other services along with Notfellchen or you have already set up services which need Valkey (such as [Nextcloud](nextcloud.md), [PeerTube](peertube.md), and [Funkwhale](funkwhale.md)), it is recommended to install a Valkey instance dedicated to Notfellchen. See [below](#setting-up-a-shared-valkey-instance) for an instruction to install a shared instance.
...
At first, you need to adjust `inventory/hosts` file to add a supplementary host for Notfellchen. See [here](../running-multiple-instances.md#re-do-your-inventory-to-add-supplementary-hosts) for details.

...
# Make sure the notfellchen API service (mash-notfellchen.service) starts after the shared Valkey service (mash-valkey.service)
...
After installation, the Notfellchen instance becomes available at the URL specified with `notfellchen_hostname`. With the configuration above, the service is hosted at `https://notfellchen.example.com`.
